  ...
  return (
    <aside className="w-80 bg-dark-tertiary border-l border-dark h-full overflow-hidden flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {analyzingParagraphs.map(paragraphId => (
          <div key={`loading-${paragraphId}`} className="bg-gray-900/50 rounded-lg p-4 border border-purple-500/20">
            <div className="flex items-center gap-3">
              <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-purple-500"></div>
              <p className="text-gray-400 text-sm">
                Analyzing paragraph {paragraphId}...
              </p>
            </div>
          </div>
        ))}
